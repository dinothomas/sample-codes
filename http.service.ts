...
export class HttpService {
    ...
    public setLoggedIn(userData: any) {
        ...
        localStorage.setItem('token', userData.data.token.access_token);
        localStorage.setItem('refresh_token', userData.data.token.refresh_token);
        ...
        if (loginSuccess) {
            ...
            const that = this;
            setTimeout(function(){
                that.router.navigate(['dashboard/care-providers-list']);
            }, 500);
        }
    }
    ...
    public isLoggedIn() {
      return (localStorage.getItem('token') && localStorage.getItem('user_data')) ? true : false;
    }
    ...
    public putTest(url, data): Observable<any> {
        this.requestInterceptor();
    ...
    public delete(url, data): Observable<any> {
        ...
        return this.http.request( 'delete', this.getBaseUrl(url), { body: data })
            .map(this.extractData)
            .catch(this.handleError)
            .do((res: Response) => {
                this.onSubscribeSuccess(res, true);
    ...
    public uploadPost(url, dataToSend): Observable<any> {
        ...
        return this.http.post(this.getBaseUrl(url), dataToSend.formData)
            .map(this.extractData)
            .catch(this.handleError)
            .do((res: Response) => {
                this.onSubscribeSuccess(res, true);
    ...
    public uploadPut(url, formData): Observable<any> {
        ...
        return this.http.put(this.getBaseUrl(url), formData)
            .map(this.extractData)
            .catch(this.handleError)
            .do((res: Response) => {
                this.onSubscribeSuccess(res, true);
    ...
    private extractData(res: Response) {
        ...
        //     return {};
        // }
        return res; // after interceptor added
    }
